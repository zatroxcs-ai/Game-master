function refreshGameData() {
    ...
    function renderCurrentDeck(divId) { 
        ...
        currentDeck.forEach((cid, idx)=>{ 
            ...
            c.innerHTML+=`<div class="cr-card-mini"><img src="${card.img}"><div class="remove-card-btn" onclick="removeCard(${idx},'${divId}')">x</div></div>`; 
        }); 
    }
    ...
    function loadRelationsFor(id, name) { 
        ...
        [...gameData.players,...gameData.npcs].forEach(t=>{ 
            ...
            <div class="relation-target">
                <b>${t.name}</b>
                <select onchange="updateRel(${id},${t.id},'status',this.value)">
                ...
                <input type="text" value="${r.note}" onchange="updateRel(${id},${t.id},'note',this.value)">
            </div>`; 
        }); 
    }
    ...
    function newJournalForm() { 
        ...
        // This was missing in your provided code block, re-adding it here
        renderParticipants([]); 
    }
	...
	// === SURCHARGE : AFFICHAGE DECK AVEC INFOS (HOVER) ===
    // Cette fonction remplace l'ancienne renderCurrentDeck pour ajouter le menu contextuel
    function renderCurrentDeck(divId) { 
        const c = document.getElementById(divId); 
        c.innerHTML = ""; 
        
        currentDeck.forEach((cid, idx) => { 
            // On cherche les infos complètes de la carte
            const card = gameData.cards.find(x => x.id === cid) || {
                name: 'Inconnu', img: '', desc: 'Pas de description', elixir: '?', type: '?'
            }; 

            // On construit le HTML de la bulle d'info
            const tooltipHtml = `
	...
	// === SURCHARGE : RELATIONS VISUELLES ===
    
    // Nouvelle version de la fonction d'affichage des relations
    function loadRelationsFor(id, name) { 
        selectedRelationSubject = id;
        document.getElementById('relTitle').innerText="Relations de "+name; 
        const c=document.getElementById('relContainer'); c.innerHTML=""; 
        ...
        [...gameData.players,...gameData.npcs].forEach(t=>{ 
            ...
            // Nettoyage du statut pour le CSS (enlève les accents : Allié devient Allie)
            const statusCss = r.status.normalize("NFD").replace(/[\u0300-\u036f]/g, "");

            // On ajoute la barre de couleur au début et on utilise la nouvelle fonction d'update sur le select
            c.innerHTML+=`
    ...
    // Nouvelle fonction pour mettre à jour la couleur instantanément
    function updateRelWithVisual(from, to, field, selectElement) {
        const val = selectElement.value;
        // 1. Sauvegarder les données (appelle l'ancienne fonction)
        updateRel(from, to, field, val);

        // 2. Mettre à jour le visuel immédiatement
        const row = document.getElementById(`rel-row-${to}`);
        const bar = row.querySelector('.rel-status-bar');
        if(bar) {
            // Nettoyer le nom pour le CSS
